...
func testSessionExportCallback(t *testing.T, sb integration.Sandbox) {
	...
	callback := sessionexport.FinalizeExportCallback(func(resp2 map[string]string) error {
		resp = resp2
		return nil
	})
	...
	require.NotNil(t, resp, "callback not called")
	require.Equal(t, "foobar", resp["image.name"])
}
